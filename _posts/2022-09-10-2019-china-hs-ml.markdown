...
3. In the Cartesian coordinate system, $$\vec{e}$$ is the unit vector, the vector $$\vec a$$ satisfies $$\vec a\cdot \vec e=2$$, and $$\left\vert \vec a\right\vert ^2 \leq 5\left\vert \vec a + t\vec e\right\vert $$ for any real number $$t$$, find the range of possible values for $$\left\vert \vec a\right\vert $$.
...
7. As shown in the figure, a cross section of the cube $$ABCDEFGH$$ passes through the vertices $$A$$, $$C$$, and a point $$K$$ on $$EF$$, and the cube is divided into two parts with volume ratio $$3:1$$. Find the value of $$\frac{EK}{KF}$$. 
	...
	![cube diagram](/website/assets/china-cube.png)
...
11. *(20 points)* Call a sequence of complex numbers $$\{z_n\}$$ *interesting* if $$\vert z_1\vert =1$$, and for any positive integer $$n$$, we have $$4z_{n+1}^2+2z_nz_{n+1}+z_n^2=0.$$$ Find the largest constant $$C$$ such that for all interesting sequences $$\{z_n\}$$ and any positive integer $$m$$, we have $$\vert z_1+z_2+\ldots + z_m\vert \geq C.$$
...
11. *(20 points)* Let the sequence of complex numbers $$\{z_n\}$$ satisfy $$\vert z_1\vert =n$$, and, for any positive $$n$$, $$4z_{n+1}^2 + 2z_nz_{n+1}+z_n^2 = 0.$$$ Prove that, for any positive integer $$m$$, we have $$\vert z_1+z_2+\cdots + z_m\vert <\frac{2\sqrt 3}{3}.$$